...
from shared import CC_lab, ROOM_DATA_FOLDER, ROOM_STATUS_FILE_PATH
...
def allocate_rooms(rooms, students, course_name,date,time):
    ...
    for _, room in course_rooms.iterrows():
        ...
        if room_name.startswith('LT'):
            ...
            lt_soft_path =os.path.join(ROOM_DATA_FOLDER, f"{room_name}_SoftLimit.xlsx")#added

            lt_hard_path = f"data\\room_data\\{room_name}_HardLimit.xlsx"
            lt_hard_path =os.path.join(ROOM_DATA_FOLDER, f"{room_name}_HardLimit.xlsx")#added
            ...
            dlt_path = os.path.join(ROOM_DATA_FOLDER, f"{room_name}.xlsx")
...
def update_room_csv(room_name=None, capacity=0, filled_seats=0, limit_type=" ", filepath=ROOM_STATUS_FILE_PATH):
